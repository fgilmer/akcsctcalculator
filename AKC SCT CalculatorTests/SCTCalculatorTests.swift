...
class SCTCalculatorTests: XCTestCase {
    ...
    func testGetBaseYPS_ValidLevelsAndClasses() {
        let yps = calculator.getBaseYPS(level: .excellentMasters, classType: .standard)
        XCTAssertEqual(yps, [2.50, 2.70, 2.85, 3.10, 2.90])

        let ypsJumpers = calculator.getBaseYPS(level: .novice, classType: .jumpers)
        XCTAssertEqual(ypsJumpers, [2.30, 2.50, 2.75, 3.00, 2.80])
    }
